...
(defn nextTlv [[buf start endPos pTlv]]
  (let [newPos (+ start (tlv/size pTlv))]
    [buf newPos endPos (tlv/tryGetTlv buf newPos endPos)]))


(defn getTlvList [buf start maxLen]
  (let [endPos (+ start maxLen)]
    (map (fn [[_ _ _ tlv]] tlv) (take-while nextTlv? (iterate nextTlv [buf start endPos (tlv/tryGetTlv buf start endPos)])))))


(defn bufToString [buf start len]
  (apply str (map char (for [idx (range len)]
                         (tlv/getu buf (+ start idx))))))

(defn hexify [buf start len]
...
(defrecord NameSeg [ntype nval]
  Object
  (toString [nameSeg]
    (str "/" (format "%04x" ntype) "=" (format "%x" nval))))

(defn subByteArray 
  "create new byte-array from subset of buf"
  [buf start len]
...
(defn parseNameSeg
  ...
  (let [len (:length tlv)
    ...
    (case typ
      ...
      (-> NameSeg typ (subByteArray buf start len))
    ;;
      ((:T_IPID tlvTypes))
      (-> NameSeg typ (subByteArray buf start len))
      ;; default
...
(defn parseNameTlv
  "return vector of NameSeg pairs"
  [tlv]
...
(defn strToBuf [nameSeg buf start]
  (let [bytes (.getBytes nameSeg)]
    (count (for [i (range (.length nameSeg))]
      (aset-byte buf (+ start i) (get bytes i))))))


(defn copyNameSeg [nameSeg buf start bytesLeft]
...
(defn writeTlvHeader
  ...
  ([type len buf start bytesLeft] ;; should check whether bytesLeft>=tlv-header-len
   ...
   (if-let [typeVal (get tlvTypes type)]
...
(defn parseInterest
  ...
  (do
    ...
    (let [hopLimit (headerHopLimit buf)
          ...
          optTLVs (getTlvList msgBuf (+ start nameTLVlen)
