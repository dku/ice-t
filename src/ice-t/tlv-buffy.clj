...
;  (:import (java.nio ByteBuffer))
  ...
  (:require [clojurewerkz.buffy.frames :refer :all])
  )
...
(def tlv16-payload
  (dynamic-buffer
   (frame-type
    (frame-encoder [tlv]
                   type (short-type) (:type tlv)
                   length (short-type) (:length tlv)
                   value (bytes-type (:length tlv)) (:value tlv))
    (frame-decoder [buffer offset]
                   type (short-type)
                   length (short-type)
                   value (bytes-type (read length buffer offset)))
    )))
  

(def tlv16
  (spec :type (short-type)
        :len (short-type)))
