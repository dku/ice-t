...
(def tlvTypes
  {
   ...
   :T_NAME 7
   :T_GEN_NAME_COMP 8
   :T_IMPLICIT_SHA_DIGEST_COMP 1
   :T_PARA_SHA_DIGEST_COMP 2
   ;; Interest Packet
   :T_CAN_BE_PREFIX 33
   :T_MUST_BE_FRESH 18
   :T_FORWARDING_HINT 30
   :T_NONCE 10
   :T_INTEREST_LIFETIME 12
   :T_HOP_LIMIT 34
   :T_APP_PARAMS 36
   :T_INTEREST_SIG_INFO 44
   :T_INTEREST_SIG_VAL 46
   ;; Data Packet 
   :T_META_INFO 20
   :T_CONTENT 21
   :T_SIG_INFO 22
   ...
   ;; Data/MetaInfo
   :T_CONTENT_TYPE 24
   :T_FRESHNESS_PERIOD 25
   :T_FINAL_BLOCK_ID 26
   ;; Signature
   :T_SIGNATURE_TYPE 27
   :T_KEY_LOCATOR 28
   :T_KEY_DIGEST 29
   :T_SIGNATURE_NONCE 38
   :T_SIGNATURE_TIME 40
   :T_SIGNATURE_SEQ_NUM 42
   ;; Link Object
   :T_DELEGATION 31
   :T_PREFERENCE 30
   })
...
;; this should be in tlv.clj
(defn asTLVNum [num buf]
  "convert number to TLV representation writing to buf. Returns number of bytes written"
  (condp > num
    0xFD (tlv/valToBuf buf 0 num 1) 
    0XFFFF (+ (tlv/valToBuf buf 0 253 1) (tlv/valToBuf buf 1 num 2)) ;; have to set first byte!
    0xFFFFFFFF (+ (tlv/valToBuf buf 0 254 1) (tlv/valToBuf buf 1 num 4)) ;; have to set first byte!
    (+ (tlv/valToBuf buf 0 255 1) (tlv/valToBuf buf 1 num 8)))) ;; default -- have to set first byte!
    

(defn TLVNum [buf]
  "parse bytes at buf position as TLV number with unknown size. Return parsed num"
  (let [byte0 (tlv/getu buf 0)]
    (condp > byte0
      0XFD byte0
      254 (tlv/bytes2word buf 1 2)
      255 (tlv/bytes2word buf 1 4)
      (tlv/bytes2word buf 1 8))))


(defn TLV-num-len [buf]
  "get length of number field for this TLV at pos buf"
  (let [byte0 (tlv/getu buf 0)]
    (condp > byte0
      0XFD 1
      254 2
...
(defn TLV-type-len [buf]
  "get length of type field for this TLV"
  (TLV-num-len buf))

(defn TLV-len-len [buf]
  "get length of type field for this TLV"
  (TLV-num-len buf))

(defn TLV-header-len [buf]
  "get length of total header (type, length) for this TLV"
  (let [type-len (TLV-num-len buf)]
    (+ type-len (TLV-len-len (+ buf type-len)))))
        

(defn tType
  "read TLV type from byte array"
  ([array] (TLVNum array))
  ([array start] (tType (+ array start))))


(defn length
  "read TLV length from byte array"
  ([array] (TLVNum (+ array (TLV-type-len array))))
  ([array start] (length (+ array start))))


(defn tVal
  "get TLV value position in byte array"
  ([array] (.position array (TLV-header-len array)))
  ([array start] (tVal (+ array start))))




;; bytes -- do this with macros later
